...
public class SimpleRestService {
	...
	public String getSomething(@QueryParam("request") String request ,@DefaultValue("1") @QueryParam("version") int version) {
	...
	@GET
	 ...
	 public Response getLog4J(@Context ServletContext ctx) throws IOException {
			
			List<Motel> moteles = new ArrayList<Motel>();
			Motel motel = new Motel("IBIZA", "JACUZZI", 15.0, 2, "Candela");
			Motel motel1 = new Motel("CUPIDO", "TURCO", 5.0, 1, "Candelita");
			moteles.add(motel);
			moteles.add(motel1);
...
//			input = new FileInputStream(path+"/Log4j.properties");
//			propin.load(input);
//			input.close();
//			List<propertiesObject> lLista = new ArrayList<propertiesObject>(); 	
//
//			lLista.add(new propertiesObject("encabezado",propin.getProperty("encabezado")));
						
			return Response.ok( moteles ).build();
	 	}
